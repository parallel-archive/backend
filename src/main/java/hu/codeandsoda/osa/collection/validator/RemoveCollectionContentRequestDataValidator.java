...
@Component
public class RemoveCollectionContentRequestDataValidator {

    private static Logger logger = LoggerFactory.getLogger(RemoveCollectionContentRequestDataValidator.class);
    ...
    public void validate(Object target, Errors errors) {
        ...
        if (collectionExists && publishedDocumentExists) {
    ...
    private boolean validateIfPublishedDocumentExists(String publishedDocumentHash, Long collectionId, Long userId, Errors errors) {
        ...
        if (!publishedDocumentExists) {
            ...
            String logMessage = MessageFormat.format(
                    "action=removePublishedDocumentFromCollection, status=error, userId={0}, publishedDocumentHash={1}, collectionId=@{2}, errorCode={3}", userId,
    ...
    private void validateIfCollectionContainsPublishedDocument(Collection collection, String publishedDocumentHash, Long collectionId, Long userId, Errors errors) {
        ...
        boolean collectionContainsDocument = documents.stream().anyMatch(d -> d.getHash().equals(publishedDocumentHash));
        if (!collectionContainsDocument) {
