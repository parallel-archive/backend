...
@Entity
public class PublishedDocumentMetaData implements Serializable {
    ...
    @JoinTable(name = "document_tag_published_document_meta_data", joinColumns = @JoinColumn(name = "published_document_meta_data_id", foreignKey = @ForeignKey(name = "FK_PUBLISHED_DOCUMENT_META_DATA_OF_TAG_ID")), inverseJoinColumns = @JoinColumn(name = "document_tag_id", foreignKey = @ForeignKey(name = "FK_PUBLISHED_DOCUMENT_TAG_ID")))
    private List<DocumentTag> tags;

    @ManyToMany(fetch = FetchType.EAGER)
    @Fetch(value = FetchMode.SUBSELECT)
    @JoinTable(name = "published_document_meta_data_document_filter", joinColumns = @JoinColumn(name = "published_document_meta_data_id", foreignKey = @ForeignKey(name = "FK_PUBLISHED_DOCUMENT_META_DATA_OF_DOCUMENT_FILTERS_ID")), inverseJoinColumns = @JoinColumn(name = "document_filter_id", foreignKey = @ForeignKey(name = "FK_PUBLISHED_DOCUMENT_FILTER_ID")))
    private List<DocumentFilter> documentFilters;

    public PublishedDocumentMetaData() {
        tags = new ArrayList<>();
