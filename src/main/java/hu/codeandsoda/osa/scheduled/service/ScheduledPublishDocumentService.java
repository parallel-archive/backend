...
@Service
public class ScheduledPublishDocumentService {

    private static final String PUBLISH_DOCUMENT_ACTION = "Validate and set Document status to Published";

    private static Logger logger = LoggerFactory.getLogger(ScheduledPublishDocumentService.class);
    ...
    public void publishDocument() {
        ...
        if (null != publishedDocumentId) {
            ...
            String logStart = MessageFormat.format("START Publish document: {0}. Publish Document ID: {1}. Current time is: {2}", PUBLISH_DOCUMENT_ACTION,
            ...
            try {
                publishedDocumentService.setStatusToPublished(publishedDocumentId, errors);

                long duration = Duration.between(start, ZonedDateTime.now()).getSeconds();
                String logEnd = MessageFormat.format("END Publish document: {0}. Published Document ID: {1}. Duration: {2} seconds.", PUBLISH_DOCUMENT_ACTION,
                        publishedDocumentId, duration);
                logger.info(logEnd);

            } catch (Exception e) {
                jmsErrorHandler.handleDocumentPublishError(PUBLISH_DOCUMENT_ACTION, queue, publishedDocumentId, e);
            }
        } else {
